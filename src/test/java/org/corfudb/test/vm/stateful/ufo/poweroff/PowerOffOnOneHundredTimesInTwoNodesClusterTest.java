...
public class PowerOffOnOneHundredTimesInTwoNodesClusterTest extends AbstractCorfuUniverseTest {
     ...
     * <p>
     * Test cluster behavior after power off/power on two nodes in loop for 100 times
     ...
     * 6) PowerOff/PowerOn two nodes and then start corfu process on two nodes
     * 7) Wait for Cluster status (should be STABLE)
     * 8) Add 100 more entries into Table
     * 9) Repeat steps 6-8 100 Times
    ...
    private void verifyPowerOnPowerOffNode(UniverseWorkflow<Fixture<UniverseParams>> wf)
        ...
        // Define table name
        String tableName = "CorfuUFO_PowerOffOnOneHundredTimesInTwoNodesClusterTable";
        ...
        for (int loopCount = 1; loopCount <= LOOP_COUNT; loopCount++) {
            ...
            log.info(String.format("**** Verify cluster status STABLE :: after power OFF/ON two nodes %s times ****",
                    loopCount));
            waitForClusterStatusStable(corfuClient);
