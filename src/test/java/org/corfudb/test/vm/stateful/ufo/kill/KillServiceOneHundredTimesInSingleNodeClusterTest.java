...
public class KillServiceOneHundredTimesInSingleNodeClusterTest extends AbstractCorfuUniverseTest {
     ...
     * <p>
     * Test cluster behavior after kill service on one node in loop for 100 times
    ...
    private void verifyKillService(UniverseWorkflow<Fixture<UniverseParams>> wf)
        ...
        // Define table name
        String tableName = "CorfuUFO_KillServiceOneHundredTimesInSingleNodeClusterTable";
        ...
        // Loop kill service on single node after detachment of other nodes
        for (int loopCount = 1; loopCount <= LOOP_COUNT; loopCount++) {
            log.info("**** In Loop :: " + loopCount + " ****");

            log.info("**** Kill service on Single node ****");
            server0.kill();

            log.info("**** Start service on Single node ****");
            server0.start();
