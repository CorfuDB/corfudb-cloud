...
@Slf4j
public class PowerOffOnOneNodeFiveHundredTimesTest {
     ...
     * <p>
     * Test cluster behavior after power OFF/ON one node
    ...
    private void verifyVmOperations(UniverseManager.UniverseWorkflow<Fixture<UniverseParams>> wf)
        ...
        // Define table name
        String tableName = "CorfuUFO_PowerOffOnOneNodeFiveHundredTimesTable";
        ...
        for (int lcount = 1; lcount <= LOOP_COUNT; lcount++) {
            ...
            log.info("*********************");
            log.info(String.format("**** required values like start::%s, end::%s and lcount::%s ****",
                    start, end, lcount));
            log.info("*********************");
            ...
            if (lcount % 2 == 0) {
                isTrue = true;
                log.info(String.format(" **** updating the records with start::%s, end::%s and lcount::%s ****",
                        start, end, lcount));
                UfoUtils.generateDataAndCommit(start, end, tableName, uuids, events, tx, metadata, isTrue);
            }
